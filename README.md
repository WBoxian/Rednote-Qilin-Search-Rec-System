...
- `datasets/`
  - 原始训练/测试数据
  - 包含 search / recommendation 两个场景的 train / test 请求数据、用户画像、笔记内容等
- `features/`
  - 离线阶段生成的样本特征 parquet
- `embeddings/`
...
- `outputs/`
  ...
  - `models/`：模型权重
  - `index/`：召回索引与映射文件
  - `data/`：训练中间数据、候选集、hard negative 等
  - `results/`：评估结果与分析产物
  - `deploy/{scene}/{tag}/`：在线服务实际加载的模型与索引
  - `serving_cache/`：在线指标看板使用的快照缓存

### 1.2 后端

- `src/backend/offline/`
  - 离线总编排
  - 负责样本、特征、训练、部署、特征写入 Redis
- `src/backend/online/`
  - 在线服务
  - 负责 FastAPI、运行时状态管理、首页 feed、详情页、实时行为、指标看板
- `src/recall/`
  - 召回训练与索引构建脚本
  - 包括 DSSM、Faiss、Swing、UserCF、多路召回候选构建
