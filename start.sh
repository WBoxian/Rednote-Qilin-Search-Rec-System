...
BACKEND_PID="$(launch_logged "${BACKEND_LOG}" "cd '${PROJECT_ROOT}' && env PYTHONUNBUFFERED=1 TOKENIZERS_PARALLELISM=false MALLOC_ARENA_MAX=2 UV_CACHE_DIR='${UV_CACHE_DIR}' QILIN_REDIS_URL='${REDIS_URL}' QILIN_ASYNC_PREWARM=1 QILIN_BLOCKING_PREWARM=0 QILIN_PREWARM_FEED=1 QILIN_PREWARM_METRICS=0 QILIN_PREWARM_VALIDATION=0 QILIN_PREWARM_FEED_PAGE_SIZE=15 QILIN_BLOCKING_PREWARM_FEED_SCENES= QILIN_ASYNC_PREWARM_FEED_SCENES=search,rec QILIN_PREWARM_METRICS_SAMPLE_N=48 QILIN_PREWARM_VALIDATION_GROUPS=3 QILIN_PREWARM_VALIDATION_EXAMPLES=3 QILIN_TAG_SEARCH=hard QILIN_TAG_REC=hard uv run python src/backend/online/api/main.py --host '${BIND_HOST}' --port '${BIND_BACKEND_PORT}' --tag hard")"
